...
uses
  ...
  ComCtrls, StdCtrls, lcltype, Spin, ExtCtrls, Windows,
  common;
...
type
  ...
  TfrmDisplayListGen = class(TForm)
    ...
    editAddrDec: TSpinEdit;
    editAddrHex: TLabel;
    editHighByte: TSpinEdit;
    editLineStep : TSpinEdit;
    editLowByte: TSpinEdit;
    box: TGroupBox;
...
begin
  while frmDisplayList.dl[i] <> 65 do begin
    ...
    if rgDataType.ItemIndex = 0 then begin
...
//procedure TfrmDisplayListGen.tabsChange(Sender: TObject);
//begin
//  TRadioButton(rgDataType.Controls[0]).Enabled := true;
//  TRadioButton(rgDataType.Controls[1]).Enabled := true;
//
