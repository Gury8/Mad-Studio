{
  ...
  Unit: Font (character set) editor
}
unit fonts;
...
begin
  ...
  caption := programName + ' ' + programVersion + ' - Character set editor';
  frmMain.Top := 0;
  formId := formFont;
  ...
  imgFontSet.Canvas.Brush.Color := clBlack;
  imgFontSet.Canvas.FillRect(bounds(0, 0, imgFontSet.Width, imgFontSet.Height));
  imgFontSetInv.Canvas.Brush.Color := clBlack;
...
procedure TfrmFonts.btnByteEditorClick(Sender : TObject);
var
  x, y : byte;
  bin : string;
  dataValue : byte;
...
//        ShowFontSet02(offs);
    ...
    for n := 0 to 14 do begin
      if (x > 16*n) and (x <= 32 + n shl 4) and (y > m shl 4) and (y < 20 + m shl 4) then begin
        offsY := m; offsX := n;
        offs := n + m shl 4 + 1;
...
procedure TfrmFonts.LoadDefaultFont;
var
  r, i : byte;
  j : integer;
  bin : string[9];
...
procedure TfrmFonts.SaveFontProc(Sender: TObject);
var
  bin : string[9];
  j : integer;
  i : byte;
...
//procedure TfrmFonts.ShowAntic5FontSet;
//var
//  n, cnt : byte;
//  col, xf, yf, offset, xoffset, yoffset : integer;
//  mask : array[0..1] of byte;
//begin
//  imgAntic5FontSet.Canvas.Brush.Color := coltab[0];
//  imgAntic5FontSet.Canvas.Brush.Style := bsSolid;
//  imgAntic5FontSet.Canvas.FillRect(bounds(0, 0, imgAntic5FontSet.Width, imgAntic5FontSet.Height));
//
//  //imgBaseFontSet.Canvas.Brush.Color := coltab[0];
//  //imgBaseFontSet.Canvas.Brush.Style := bsSolid;
//  //imgBaseFontSet.Canvas.FillRect(bounds(0, 0, imgBaseFontSet.Width, imgBaseFontSet.Height));
...
//procedure TfrmFonts.ShowAntic7FontSet;
//var
//  n : byte;
//  col, xf, yf, offset, xoffset, yoffset : integer;
//begin
//  imgAntic7FontSet.Canvas.Brush.Color := coltab[1];
//  imgAntic7FontSet.Canvas.Brush.Style := bsSolid;
//  imgAntic7FontSet.Canvas.FillRect(bounds(0, 0, imgAntic7FontSet.Width, imgAntic7FontSet.Height));
//  offset := 0; yoffset := 0;
//
//  // Uppercase alphabet, numbers, punctuation
//  for n := 0 to 63 do begin
...
//        imgAntic7FontSet.Canvas.FillRect(bounds(xf*factX06 + xoffset, yf*factY07 + yoffset, factX06, factY07));
////        imgFontSet.Canvas.Pixels[xf*factX02, yf*factY02 + yoffset] := coltab[col];
//      end;
//    end;
//    Inc(offset);
...
//        Lines.Add('Inverse character value + 128');
  ...
  ShowAntic6FontSet02(offs, 16);  //, factY06);
  ShowAntic6FontSet02(offs, 32);  //, factY07);
...
//  RefreshAntic2Char(offs);
//  RefreshAntic6Char(offs);
end;
...
(*
procedure TfrmFonts.ShowAntic2Char(offset : integer);
var
  col, xf, yf : integer;
begin
  imgAntic2Char.Canvas.Brush.Color := clBlack;
  imgAntic2Char.Canvas.Brush.Style := bsSolid;
  imgAntic2Char.Canvas.FillRect(bounds(0, 0, imgAntic2Char.Width, imgAntic2Char.Height));

  offset := offset shl 3;

  for yf := 0 to 7 do begin
    for xf := 0 to 7 do begin
      col := fldOrig[xf, yf + offset];
      //if not isFontSetNormal then begin
      //  if col = 1 then
      //    col := 0
      //  else if col = 0 then
      ...
      //end;
      //fldChar[xf, yf] := col;
      imgAntic2Char.Canvas.Brush.Color := colTabFont[col];
      imgAntic2Char.Canvas.FillRect(bounds(xf*factOrigX, yf*factOrigY, factX, factY));
//      imgChar.Canvas.Pixels[xf*factX, yf*factY] := coltab[col];
    end;
  end;

  imgAntic2Char.Refresh;
end;

procedure TfrmFonts.RefreshAntic2Char(offset : integer);
var
  col, xf, yf : integer;
begin
  imgAntic2CharNew.Canvas.Brush.Color := clBlack;
  imgAntic2CharNew.Canvas.Brush.Style := bsSolid;
  imgAntic2CharNew.Canvas.FillRect(bounds(0, 0, imgAntic2CharNew.Width, imgAntic2CharNew.Height));
  ...
  for yf := 0 to 7 do begin
    for xf := 0 to 7 do begin
      ...
      //end;
      //fldChar[xf, yf] := col;
      imgAntic2CharNew.Canvas.Brush.Color := colTabFont[col];
      imgAntic2CharNew.Canvas.FillRect(bounds(xf*factOrigX, yf*factOrigY, factX, factY));
//      imgChar.Canvas.Pixels[xf*factX, yf*factY] := coltab[col];
    end;
  end;

  imgAntic2CharNew.Refresh;
end;
*)
...
begin
  imgAntic6Char.Canvas.Brush.Color := clBlack;
...
procedure TfrmFonts.RefreshAntic6Char(offset : integer);
var
  n : byte;
  col, xf, yf, offsetx : integer;
begin
  imgAntic6CharNew.Canvas.Brush.Color := clBlack;
  imgAntic6CharNew.Canvas.Brush.Style := bsSolid;
  imgAntic6CharNew.Canvas.FillRect(bounds(0, 0, imgAntic6CharNew.Width, imgAntic6CharNew.Height));
  offsetx := 0;
    ...
    for n := 0 to 63 do begin
      if offset = offsetx then begin
        for yf := 0 to grY02 do begin
          for xf := 0 to grX02 do begin
            col := fld[xf, n*8 + yf];
            imgAntic6CharNew.Canvas.Brush.Color := coltab[col];
            imgAntic6CharNew.Canvas.FillRect(bounds(xf*fact6OrigX, yf*fact6OrigY, fact6OrigX, fact6OrigY));
          end;
        end;
        break;
      end;
...
procedure TfrmFonts.RefreshAntic7Char(offset : integer);
var
  n : byte;
  col, xf, yf, offsetx : integer;
begin
  imgAntic7CharNew.Canvas.Brush.Color := clBlack;
  imgAntic7CharNew.Canvas.Brush.Style := bsSolid;
  imgAntic7CharNew.Canvas.FillRect(bounds(0, 0, imgAntic7CharNew.Width, imgAntic7CharNew.Height));
  offsetx := 0;
    ...
    for n := 0 to 63 do begin
      if offset = offsetx then begin
        for yf := 0 to grY02 do begin
          for xf := 0 to grX02 do begin
            col := fld[xf, n*8 + yf];
            imgAntic7CharNew.Canvas.Brush.Color := coltab[col];
            imgAntic7CharNew.Canvas.FillRect(bounds(xf*fact6OrigX, yf*fact7OrigY, fact6OrigX, fact7OrigY));
          end;
        end;
        break;
      end;
...
procedure TfrmFonts.ShowAntic7Char(offset : integer);
var
  n : byte;
  col, xf, yf, offsetx : integer;
begin
  imgAntic7Char.Canvas.Brush.Color := clBlack;
  imgAntic7Char.Canvas.Brush.Style := bsSolid;
  imgAntic7Char.Canvas.FillRect(bounds(0, 0, imgAntic7Char.Width, imgAntic7Char.Height));
  offsetx := 0;
    ...
    for n := 0 to 63 do begin
      if offset = offsetx then begin
        for yf := 0 to grY02 do begin
          for xf := 0 to grX02 do begin
            col := fldOrig[xf, n*8 + yf];
            imgAntic7Char.Canvas.Brush.Color := coltab[col];
            imgAntic7Char.Canvas.FillRect(bounds(xf*fact6OrigX, yf*fact7OrigY, fact6OrigX, fact7OrigY));
          end;
        end;
        break;
      end;
