...
type
  ...
  public
    ...
    filename : string;
    isEdit : boolean;
    procedure MemoToEditor03(Sender: TObject);
  end;
...
uses
  common, main, src_settings, pmg_gen, gr_gen, font_gen,
  antic2_gen, antic6_gen, antic4_gen, dl_gen, anim_gen;
...
begin
  ...
  if cmbLanguage.ItemIndex = 12 then begin
    if cmbProp.ItemIndex = 0 then begin
      filename := getDir + 'examples\code0.act';
      lblOutput.Caption := 'Action! output';
      btnCompile.Hint := 'Compile Action! code';
      btnCompileRun.Hint := 'Compile and run Action! code';
    end
    else begin
      filename := getDir + 'examples\code0.eff';
      lblOutput.Caption := 'Effectus output';
      btnCompile.Hint := 'Compile Effectus code';
      btnCompileRun.Hint := 'Compile and run Effectus code';
    end;
  ...
  else if cmbLanguage.ItemIndex = 13 then begin
...
begin
  ...
  cmbProp.Enabled := //(cmbLanguage.ItemIndex = 2) or
...
//      filename := copy(tempFilename, 1, length(tempFilename) - 1) + IntToStr(number) + tempExt;
  ...
  // Other languages
//  else begin
    btnCompile.Enabled := (cmbLanguage.ItemIndex <> 6) and (cmbLanguage.ItemIndex <> 7);
    btnCompileRun.Enabled := (cmbLanguage.ItemIndex <> 6) and (cmbLanguage.ItemIndex <> 7);
//  end;
...
//    0: isCRLF := true;
//    1: isCRLF := true;
//    2: isCRLF := false;  //(cmbLanguage.ItemIndex = 2) and (cmbProp.ItemIndex = 1);
//    3: isCRLF := true;
//    4: isCRLF := true;
//    5: isCRLF := true;
//    6: isCRLF := false;
...
begin
  ...
  //  filename := ExtractFileNameWithoutExt(filename) + '.pas';
...
procedure TfrmSrcEdit.MemoToEditor03(Sender: TObject);
begin
  //Atari BASIC / Turbo BASIC XL
  //Action! / Effectus
  //Mad Pascal
...
//      cmbProp.ItemIndex := 1;
    ...
    _MAC65: cmbLanguage.ItemIndex := 6;
  end;
