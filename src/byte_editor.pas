...
procedure TfrmByteEditor.CloseProc(Sender : TObject);
begin
  isDataExport := false;
  Close;
end;

procedure TfrmByteEditor.ExportDataProc(Sender : TObject);
begin
  isDataExport := true;
...
*)
  with paintBox do begin
    ...
    if not chkRowSelect.Checked then begin
      y := 0;
      for Row := strGrid.Selection.Top to strGrid.Selection.Bottom do
