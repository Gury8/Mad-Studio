...
uses
  Classes, Windows, SysUtils, FileUtil, Forms, Controls, Graphics, Dialogs, StdCtrls, ComCtrls,
  ExtCtrls, lcltype, ActnList, BCTrackbarUpdown, BCMDButton, BCListBox, BCMaterialDesignButton,
  strutils,
  common;
...
type
  ...
  TfrmPmgGen = class(TForm)
    ...
    procedure pmColorDecMouseUp(Sender : TObject; Button : TMouseButton; Shift : TShiftState;
      X, Y : Integer);
...
begin
  ...
  SetTrackBarUpDown(pmColorDec, $00DDDDDD, clWhite);
  SetTrackBarUpDown(editPosX, $00DDDDDD, clWhite);
  SetTrackBarUpDown(editPosY, $00DDDDDD, clWhite);
...
//  pmColor.Brush.Color := colTab[radPM.ItemIndex + 4];
  //editPosX.Value := colorValues[5];
  //pmColor1.Brush.Color := colTab[5];
  //editMissilePosX.Value := colorValues[6];
...
begin
  ...
  //  radLang.Controls[i].Enabled := listings[ListExamples.ListBox.ItemIndex, i];
  //end;
...
procedure TfrmPmgGen.pmColorDecMouseUp(Sender : TObject;
...
//    code := 'const'#13#10;
  ...
  { KickC
   ---------------------------------------------------------------------------}
    ...
    for i := 0 to 3 do
      if chkPlayers.Checked[i] then
        code.line += 'const char player' + IntToStr(i) + '[] = {' + SetValues(i) + '};'#13#10;
  end;
    ...
    if (ListExamples.ListBox.ItemIndex in [3, 5]) and not chkPriority.Checked[2] then begin
      for i := 0 to 3 do
        if chkMissiles.Checked[i] then begin
          ...
          code.line += CodeLine('READ A:POKE PMGMEM+' + PMBASEStart +
                                '+MY' + IntToStr(i) + '+I,A');
          code.line += CodeLine('NEXT I');
        end;
    end
...
procedure TfrmPmgGen.editStartLineMouseUp(Sender : TObject; Button : TMouseButton;
  Shift : TShiftState; X, Y : Integer);
begin
  CreateCode;
end;

procedure TfrmPmgGen.editPosXMouseUp(Sender : TObject; Button : TMouseButton; Shift : TShiftState;
  X, Y : Integer);
begin
  frmPmg.arrPosX[radPM.ItemIndex] := editPosX.Value;
  CreateCode;
...
procedure TfrmPmgGen.editPosYMouseUp(Sender : TObject; Button : TMouseButton; Shift : TShiftState;
  X, Y : Integer);
begin
  frmPmg.arrPosY[radPM.ItemIndex] := editPosY.Value;
  CreateCode;
...
procedure TfrmPmgGen.editMissilePosXMouseUp(Sender : TObject; Button : TMouseButton;
  Shift : TShiftState; X, Y : Integer);
begin
  frmPmg.arrMissilePosX[radPM.ItemIndex] := editMissilePosX.Value;
  CreateCode;
...
procedure TfrmPmgGen.editMissilePosYMouseUp(Sender : TObject; Button : TMouseButton;
  Shift : TShiftState; X, Y : Integer);
begin
  frmPmg.arrMissilePosY[radPM.ItemIndex] := editMissilePosY.Value;
  CreateCode;
