...
//    varBtn := mbLeft;
  ...
  case modeType of
    ...
    8: PlotByte(xf, yf, 20, clBlack, true);
    9: PlotByte(xf, yf, 27, clBlack, true);
...
begin
  PlotByte(xxx, yyy + 1, 27, clBlack, true);
  btn := mbMiddle;
end;
...
//      isJVB := true;
    ...
    for n := 0 to 7 do begin
      if dl[i] = n shl 4 then begin
        PlotByte(0, yf, 20 + n, clBlack, false);
...
//        end;
    ...
    (*
      ...
      PlotByte(0, yf, 20, clBlack, false);
...
begin
  { Blank scan lines
   ---------------------------------------------------------------------------}
  xf := 0; yf := 0;
//  PlotByte(xf, yf, 2, clBlack, false);  //PlotLine01(xf, yf, 7, true);
//  PlotByte(xf + 100, yf, 3, coltab[3], false);  //Plot3(xf, yf, true);
  factX02mm := 3;
  imgSelection.Canvas.Brush.Color := clBlack;
...
//  fldAtascii[xx + yy*maxX] := offs2;
  ...
  //imgEditor.Canvas.Brush.Color := clBlack;
...
//  imgEditor.Canvas.FillRect(bounds(0, yf*factY, imgEditor.Width, 3*AnticModes[6].scanLines));
  ...
  if anticMode = 2 then
    ...
    imgSelected.Canvas.Brush.Color := clBlack;
