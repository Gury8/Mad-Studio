...
type
  ...
  TfrmAntic3 = class(TForm)
    ...
    procedure imgChar1MouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState;
      X, Y: Integer);
    procedure imgChar1MouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
    procedure imgChar1MouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState;
      X, Y: Integer);
...
//        if isDefault then begin
        imgFontSet.Canvas.Brush.Color := coltabFont[0];
        imgFontSet.Canvas.FillRect(bounds(
...
//        end;
    ...
    if n >= 96 then
      for xf := 0 to grX02 do begin
        imgFontSet.Canvas.Brush.Color := coltabFont[row01[xf]];
        imgFontSet.Canvas.FillRect(bounds(
          xf*factX + xoffset, factY shl 3 + yoffset, factX, factY));
        imgFontSet.Canvas.Brush.Color := coltabFont[row02[xf]];
        imgFontSet.Canvas.FillRect(bounds(
          xf*factX + xoffset, 9*factY + yoffset, factX, factY));
        imgFontSet.Canvas.Brush.Color := coltabFont[0];
        imgFontSet.Canvas.FillRect(bounds(
          xf*factX + xoffset, yoffset, factX, factY));
        imgFontSet.Canvas.FillRect(bounds(
          xf*factX + xoffset, factY + yoffset, factX, factY));

        imgFontSetInv.Canvas.Brush.Color := coltabFont[row03[xf]];
        imgFontSetInv.Canvas.FillRect(bounds(
          xf*factX + xoffset, factY shl 3 + yoffset, factX, factY));
        imgFontSetInv.Canvas.Brush.Color := coltabFont[row04[xf]];
        imgFontSetInv.Canvas.FillRect(bounds(
          xf*factX + xoffset, 9*factY + yoffset, factX, factY));
        imgFontSetInv.Canvas.Brush.Color := coltabFont[1];
        imgFontSetInv.Canvas.FillRect(bounds(
...
begin
  frmAntic3Code := TfrmAntic3Code.Create(Self);
  with frmAntic3Code do
    try
      ShowModal;
    finally
      Free;
...
//procedure TfrmAntic3.ColorsProc(Sender: TObject);
//begin
//  frmColors.Show;
//end;
...
procedure TfrmAntic3.MenuItem2Click(Sender: TObject);
begin
  LoadFontProc(Sender);
end;

procedure TfrmAntic3.MenuItem5Click(Sender: TObject);
begin
  SaveFontProc(Sender);
end;

procedure TfrmAntic3.MenuItem7Click(Sender: TObject);
begin
  DefaultFontSet(fldFontSet);
  ShowFontSet;
//  imgChar1MouseLeave(Sender);
end;
...
var
  col, xf, yf : integer;
begin
  ShapeClear(imgChar, clBlack);
  ShapeClear(imgChar1, clBlack);
...
//      fldChar02[xf, yf] := col;
      imgChar1.Canvas.Brush.Color := coltabFont[col];
      imgChar1.Canvas.FillRect(bounds(xf*factX04, yf*factY04, factX04, factY04));
    end;
...
procedure TfrmAntic3.imgChar1MouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  xf, yf : byte;
begin
...
procedure TfrmAntic3.imgChar1MouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var
  xf, yf : byte;
begin
  xf := X div factX04;
  yf := Y div factY04;
  PlotChar(xf, yf);
  statusBar.Panels[2].Text := 'Character editor cursor coordinates: ' +
                              'x: ' + inttostr(xf) + ', y: ' + inttostr(yf);
end;

procedure TfrmAntic3.imgChar1MouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  btn02 := mbMiddle;
  RefreshChar;
...
//            if (j = n*10) or (j = n*10 + 1) then begin
//            end
//            else begin
//              bin := '';
//              for i := 0 to 7 do begin
//                bin += IntToStr(fld[i, j]);
//              end;
//              if j = n*10 + 2 then begin
//                rows[2] := bin;
//              end
//              else if j = n*10 + 3 then begin
//                rows[3] := bin;
//              end
//              else if j = n*10 + 4 then begin
//                rows[4] := bin;
//              end
//              else if j = n*10 + 5 then begin
//                rows[5] := bin;
//              end
//              else if j = n*10 + 6 then begin
//                rows[6] := bin;
//              end
//              else if j = n*10 + 7 then begin
//                rows[7] := bin;
//              end
//              else if j = n*10 + 8 then begin
//                rows[0] := bin;
//              end
//              else if j = n*10 + 9 then begin
//                rows[1] := bin;
//              end;
//              //for i := 0 to 7 do begin
//              //  fs.WriteByte(bin2dec(rows[i]));
//              //end;
////            end;
          end;
      end;
      *)
      for j := 0 to 127 do begin
        //for i := 0 to 7 do begin
        //  fldFontSet[i, 0 shl 3 + j] := fld[i, 0 * 10 + j];
        //end;
        //for i := 0 to 7 do begin
        //  fldFontSet[i, 1 shl 3 + j] := fld[i, 1 * 10 + j];
        //end;
        if j < 96 then begin
          for n := 0 to 7 do
            for i := 0 to 7 do
