...
type
  ...
  TfrmFontSetGen = class(TForm)
    ...
    procedure tvExamplesClick(Sender: TObject);
  private
...
begin
  ...
  if radLang.ItemIndex < 2 then begin
    ...
    code.line := IntToStr(code.number) + ' DATA ' +
                 SetDataValues(frmFonts.fldChar, frmFonts.fld, 255, rgDataType.ItemIndex, ',') +
                 #13#10;
  end
  { Action!
   ---------------------------------------------------------------------------}
  ...
  { Mad Pascal
   ---------------------------------------------------------------------------}
    ...
    code.line := 'const' + #13#10 +
            ...
            '    (' + SetDataValues(frmFonts.fldChar, frmFonts.fld, 255, rgDataType.ItemIndex, ', ') + ');' + #13#10;
...
//    SetValues(_FAST_BASIC, 255)
  ...
  { Mad Assembler
   ---------------------------------------------------------------------------}
    ...
    code.line := 'chrData .BYTE ' + SetDataValues(frmFonts.fldChar, frmFonts.fld, 255, rgDataType.ItemIndex, ',') + #13#10;
//    SetValues(_MADS, 255)
  end
  { MAC/65
   ---------------------------------------------------------------------------}
  ...
  if radLang.ItemIndex = _ATARI_BASIC then begin
    ...
    for i := 0 to 127 do
      if frmFonts.charEditIndex[i] = 1 then begin
        ...
        code.line += IntToStr(code.number) + ' DATA ' +
                     SetDataValues(frmFonts.fldChar, frmFonts.fld,i, rgDataType.ItemIndex, ',') +
                     #13#10;
        Inc(code.number, code.step);
      end;
  end
  ...
  if radLang.ItemIndex < 2 then begin
    ...
    for i := 0 to 127 do begin
      ...
      code.line += CodeLine('DATA ' + SetDataValues(
                              frmFonts.fldChar, frmFonts.fld, i, rgDataType.ItemIndex, ','));
  ...
  { Mad Assembler (MADS)
   ---------------------------------------------------------------------------}
    ...
    code.line := '; Character set data'#13#10 +
            'fontData';
    for i := 0 to 127 do begin
      code.line += #13#10' .BYTE ' + SetDataValues(frmFonts.fldChar, frmFonts.fld, i, rgDataType.ItemIndex, ',');
//      SetValues(_MADS, i)
    end;
  end
  { MAC/65
...
//      for j := 0 to 7 do begin
      temp := SetDataValues(frmFonts.fldChar, frmFonts.fld, i, rgDataType.ItemIndex, ', ');
//        SetValues(_CC65, i)
...
procedure TfrmFontSetGen.radLangProc(Sender: TObject);
begin
  //langIndex := radLang.ItemIndex;
  //if langIndex = 0 then begin
  //  rgDataType.ItemIndex := 0;
...
procedure TfrmFontSetGen.tvExamplesClick(Sender: TObject);
var
  i : byte;
begin
//  showmessage(inttostr(tvExamples.selected.AbsoluteIndex));
